...
from tests.final_demo.services.search_query_engine import (  # noqa: E402
    DEFAULT_DEMO_QUERIES,
    build_search_query_outputs,
    update_run_manifest_for_search_query_engine,
)
from tests.final_demo.services.video_io import write_json  # noqa: E402


def run_step_09_search_query_engine(
    run_dir: Path,
    queries: list[str] | None = None,
    *,
    debug_full: bool = False,
) -> dict[str, Any]:
    print("[final-demo] Starting Step 9: search query engine + result grouper")

    selected_queries = list(queries or DEFAULT_DEMO_QUERIES)
    step_result = build_search_query_outputs(run_dir, selected_queries, debug_full=debug_full)
    results_path = run_dir / "09_search_results.json"
    report_path = run_dir / "09_search_query_report.json"
    debug_path = run_dir / "09_search_debug_matches.json"
    ...
    update_run_manifest_for_search_query_engine(run_dir / "run_manifest.json")

    report_payload = step_result["report_payload"]
    print(f"[final-demo] Loaded index record count: {report_payload['index_records_loaded']}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run final_demo Step 9 search query engine.")
    ...
    if not run_dir_value:
        raise ValueError(
            "Provide --run-dir or set FINAL_DEMO_RUN_DIR before running Step 9 directly."
        )
