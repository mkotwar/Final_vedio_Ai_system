...
class TrackingPersistenceService:
    ...
    def prepare_track_write(self, track: LocalVehicleTrack) -> PreparedTrackWrite:
        ...
        runtime_class_name = normalize_runtime_vehicle_class(track.stable_class_name or "unknown")
        if runtime_class_name is None:
            raise PersistenceValidationError(f"Unsupported vehicle class: {track.stable_class_name or 'unknown'}")
        track_record = VehicleTrackRecord(
            camera_id=camera_id,
    ...
    def _validate_track(self, track: LocalVehicleTrack) -> None:
        ...
        if not is_supported_vehicle_class(persisted_class_name):
            raise PersistenceValidationError(f"Unsupported vehicle class: {persisted_class_name}")
