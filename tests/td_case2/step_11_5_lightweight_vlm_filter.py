...
def _load_model_components(filter_config: dict[str, Any]) -> tuple[Any, Any, Any, float, str, float | None]:
    ...
    device = str(filter_config["device"] or "auto").strip().lower()
    dtype = torch.float32
    ...
    if device == "auto":
        ...
        if not torch.cuda.is_available():
            raise RuntimeError("TD_CASE2_STEP11_5_DEVICE=cuda requested, but CUDA is not available.")
        device_map = "auto"
        dtype = torch.bfloat16 if torch.cuda.is_bf16_supported() else torch.float16
    elif device != "cpu":
        raise ValueError(f"Unsupported Step 11.5 device: {device!r}")
