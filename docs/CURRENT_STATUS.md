...
Recent Florence colour adjustment: the multicamera pipeline now uses the verified td_case2-equivalent Florence VQA colour prompt so the known white-car `best_overall.jpg` crop for `RUN_20260727_184525:CAM_003:TRACK_10` no longer flips to `blue` under the newer JSON-style prompt.
Recent evidence adjustment: the multicamera tracking pipeline now saves padded vehicle crops, role-aligned original full frames, and role-aligned YOLO-annotated full frames under per-track artifact folders, with edge-clipping diagnostics and a manifest that records the source frame, annotated frame, crop paths, and bbox metadata for each selected evidence role.
Recent startup and media-repair adjustment: the multicamera app now includes a dedicated `scripts/start_api.ps1` launcher for the FastAPI bridge on `127.0.0.1:8000`, clearer frontend search-failure messaging that distinguishes backend-unavailable versus backend-service errors, an additive `030_track_media_full_frame_types.sql` migration for `FULL_FRAME` and `ANNOTATED_FULL_FRAME`, and a dry-run-safe `backfill_full_frame_media.py` repair script for runs such as `RUN_20260728_113958` where live `analytics.track_media` batch inserts were rejected by the older `chk_track_media_type` constraint.
Recent tracking-documentation adjustment: the multicamera pipeline now includes `scripts/report_tracking_configuration.py` for read-only effective-config reporting plus `docs/TRACKING_REFERENCE.md`, which traces the current worker tracking flow, per-camera ByteTrack isolation, lifecycle/class-stabilization behavior, identity-continuity splits, fragment linking, evidence selection, parameter sources, and safe debugging commands against the actual July 28, 2026 code.
Recent tracking-experiment adjustment: the multicamera pipeline now includes `scripts/run_tracking_confidence_experiment.py` plus focused helper coverage for controlled CAM_002 detector-confidence sweeps, per-frame detection CSVs, per-track class-history/persistence diagnostics, evidence indexes, and report-only class-wise bbox aspect-ratio analysis. On Tuesday, July 28, 2026, the scripted 20-second CAM_002 sweep over confidence `0.25`, `0.35`, `0.40`, `0.50`, and `0.60` completed under `debug_runs/multicamera_vehicle_tracking_pipeline/confidence_sweep/`; the current automatic heuristic selected `0.60` as the cleanest of those tested values because it reduced the sweep from 13 logical tracks / 8 mixed tracks / 1 plate conflict at `0.25` to 4 logical tracks / 2 mixed tracks / 0 plate conflicts at `0.60`, although motorcycle recall dropped to zero at `0.50` and `0.60`, so manual artifact review remains required before any production-threshold change.
Recent class-consistency adjustment: the multicamera tracking lifecycle now requires measurable class consistency before finalizing a stable class, reports count-vs-score winner diagnostics plus recent-window and transition metrics, and distinguishes repeated incompatible class sequences from normal one-frame label noise. On Tuesday, July 28, 2026, the fixed-confidence `0.38` CAM_002 class-consistency experiment completed under `debug_runs/multicamera_vehicle_tracking_pipeline/class_consistency_experiment/` for threshold sets A (`3 / 0.60 / 2`), B (`4 / 0.70 / 3`), and C (`5 / 0.80 / 3`); all three runs produced 3 stable classes, 6 unknown-or-unstable tracks, 1 mixed car/truck track, 2 strong conflicts, and 2 split recommendations with no automatic split execution yet, so the new diagnostics are working while further transition-specific evidence review remains the next tracking priority.
Recent logical-split adjustment: the multicamera lifecycle now converts a confirmed incompatible-class conflict into a real logical-track split when repeated conflict confidence and geometric discontinuity agree. The split is cut at the first conflicting observation, the completed parent track retains only its pre-conflict observations, the child track receives the trailing conflicting sequence under a new logical track ID and UUID, and both tracks now carry explicit split metadata such as split frame, reason codes, source/new logical IDs, native tracker ID, and discontinuity metrics. Focused regression coverage on Tuesday, July 28, 2026 confirmed that the split path preserves all observations without duplication across the old/new logical tracks while keeping the same native tracker ID available in diagnostics.
Recent class-threshold adjustment: the multicamera detector now supports optional class-specific confidence thresholds without changing the YOLO/ByteTrack architecture. When enabled, YOLO inference runs once per frame at the minimum configured class threshold, then normalized detections are filtered per class before they reach ByteTrack. On Tuesday, July 28, 2026, the CAM_002 class-confidence experiment completed under `debug_runs/multicamera_vehicle_tracking_pipeline/class_confidence_experiment/` for the global `0.38` baseline plus threshold sets A, B, and C. The balanced Test B (`car=0.38`, `truck=0.50`, `bus=0.50`, `motorcycle=0.32`, `3wheeler=0.38`) reduced mixed tracks from `5` to `4`, cut accepted truck detections from `40` to `34`, cut accepted bus detections from `14` to `9`, and preserved the same accepted motorcycle count (`3`) as the baseline, making it the current best candidate for manual artifact review before any permanent detector-threshold change.
Recent mixed-identity adjustment: the multicamera tracking lifecycle now flags count-vs-score disagreement across substantial incompatible time segments as `MIXED_IDENTITY`, blocks stable final-class assignment until a real split happens, and exposes the new diagnostics through track JSON, class-consistency reports, and evidence labels. The td_case2 experiment layer also now includes `scripts/run_final_class_parameter_matrix.py` for the staged 19-run CAM_002 parameter matrix covering class-specific thresholds, class-consistency thresholds, split thresholds, and NMS IoU without changing the tracking architecture.
Recent standard-tracking adjustment: the multicamera pipeline now has an explicit `tracking.behavior_mode` split between `experimental_custom` and `standard_bytetrack`, plus a simple `track_class` weighted-majority vote for clean ByteTrack-only validation. On Tuesday, July 28, 2026, the new `run_standard_bytetrack_comparison.py` A/B run over CAM_002 frames `0-599` at confidence `0.38` and NMS IoU `0.45` completed under `debug_runs/multicamera_vehicle_tracking_pipeline/standard_bytetrack_comparison/`; both modes produced `9` native / `9` logical tracks with `1` mixed car/truck track and `2` mixed car/bus tracks, while standard mode surfaced `4` unknown final classes versus `2` in the experimental mode instead of forcing extra class certainty through custom logic.
Recent detector-threshold cleanup: the normal multicamera validation entrypoints no longer accept a separate detector-confidence override for vehicle detection. As of Tuesday, July 28, 2026, `validate_multicamera_detection.py` and `validate_multicamera_tracking.py` now take class-specific vehicle thresholds only from `tests/td_case2/multicamera_vehicle_tracking_pipeline/config/detection.yaml`, while their reports expose the per-class threshold map plus the derived inference floor used for the single YOLO forward pass.
